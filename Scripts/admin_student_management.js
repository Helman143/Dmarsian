document.addEventListener('DOMContentLoaded', function() {
    ...
    tableBody.addEventListener('click', function(e) {
        ...
        // Normalize STD number shown in the table (which omits the STD- prefix)
        (function(){
            const raw = (cells[0].textContent || '').replace(/^STD-?/i, '');
            const padded = String(parseInt(raw || '0', 10)).padStart(5, '0');
            document.querySelector('[name="jeja_no"]').value = `STD-${padded}`;
        })();
