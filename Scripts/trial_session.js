// trial_session.js - Optimized with SweetAlert2 and Real-time Updates
...
// Re-initialize handlers (useful if table is re-rendered or items added)
function initHandlers() {
    ...
    document.querySelectorAll('#pendingTableBody .btn-approve:not(.btn-complete)').forEach(function(btn) {
        ...
        newBtn.addEventListener('click', function() {
            // Pending convert needs more info if we want to bypass 'complete' status,
            // but currently the system seems to want it 'complete' first?
            // Let's check registrations table. If it's in JSON, it's not yet in registrations.
            // convert_trial_to_student.php expects a registration_id from the DB.
            // So we MUST complete it first.
            Swal.fire({
                title: 'Note',
...
async function handleComplete(idx, row) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            // Note: Since the Complete section is populated by PHP, we can't easily add the row 
            // without a refresh or complex JS rendering. 
            // BUT, if the user wants "speed", we can at least remove the pending one and 
            // show a "Refresh to see complete list" or just refresh the second table via AJAX if we had an endpoint.
            // For now, removing the row makes it "Faster".
            setTimeout(() => { location.reload(); }, 1500); // Gradual reload to refresh the second table
        } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (searchComplete) {
        searchComplete.addEventListener('input', function(e) {
